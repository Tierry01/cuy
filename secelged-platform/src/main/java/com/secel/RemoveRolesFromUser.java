...
public class RemoveRolesFromUser extends DeclarativeWebScript{
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req,
		...
		for (int i = 0 ; i < roles.length(); i++) {
			...
			try {
				/**
				 * TODO: inherited roles may be an issue. What to do in such case: Remove the authority from
				the parent authority from which it has inherited the role
				**/

				authorityService.removeAuthority("GROUP_site_"+siteId+"_"+role, authority);
			} catch(Exception E) {
	        	status.setCode(400);
	            status.setMessage("Une Erreur est survenue lors de la suppression! Veuillez reéssayer s'il vous plait ");
	            status.setRedirect(true);
	            return model;
			}
		}

		// get & return new user roles list
